...
// FIXME: commented what follows without checking how it works
    /*
...
//    }
	...
	for (unsigned i = 0; i < BUFFER_SIZE; ++i) {
		if (out_fist[i].operation == DATA_COLLECTION) {
			already_printed = 0;
			fprintf(stderr, "%.8f %.8f %.8f %.8f\n", out_fist[i].time, out_fist[i].data, out_fist[i].omega, out_fist[i].target_power);
		} else {
