...
const TaskType BRO_Comm = 0;
const TaskType DisplayTask = 1;

extern void TASKNAME( BRO_Comm )( void );
extern void TASKNAME( DisplayTask )( void );

static __STK_UNIT _stack_BRO_Comm[__TCOUNT_STK_UNIT(512)];
...
const FP tinib_task[TNUM_TASK] = { TASKNAME( BRO_Comm ), TASKNAME( DisplayTask ), };
const __STK_UNIT tinib_stk[TNUM_TASK] = { (__STK_UNIT)_stack_BRO_Comm, (__STK_UNIT)_stack_DisplayTask, };
...
const AlarmType BRO_Comm_alarm = 0;
const AlarmType lcd_update_alarm = 1;

DeclareTask(BRO_Comm);
static void _activate_alarm_BRO_Comm_alarm( void );
static void _activate_alarm_BRO_Comm_alarm( void )
{ (void)ActivateTask( BRO_Comm ); }

DeclareTask(DisplayTask);
static void _activate_alarm_lcd_update_alarm( void );
static void _activate_alarm_lcd_update_alarm( void )
{ (void)ActivateTask( DisplayTask ); }

const CounterType alminib_cntid[TNUM_ALARM] = { 0, 0, };
const FP alminib_cback[TNUM_ALARM] = { _activate_alarm_BRO_Comm_alarm, _activate_alarm_lcd_update_alarm, };
const AppModeType alminib_autosta[TNUM_ALARM] = { 0x00000001, 0x00000001, };
const TickType alminib_almval[TNUM_ALARM] = { 1, 1, };
const TickType alminib_cycle[TNUM_ALARM] = { 5, 100, };
